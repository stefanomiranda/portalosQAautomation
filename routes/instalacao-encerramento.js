// routes/instalacao-encerramento.js
// Endpoints HTTP da esteira Instalação com Encerramento Externo.

const express = require('express');
const router  = express.Router();

const jobs         = require('../core/instalacao-encerramento-externo/jobs');
const persistence  = require('../core/instalacao-encerramento-externo/persistence');
const runner       = require('../core/instalacao-encerramento-externo/runner');
const { normalizeSa } = require('../core/instalacao-encerramento-externo/utils');
...
router.get('/health', (_req, res) => {
  res.json({ ok: true, module: 'instalacao-encerramento', ts: new Date().toISOString() });
});
...
router.post('/iniciar', async (req, res) => {
  ...
  const onLog = (msg) => {
    console.log(`[instalacao-encerramento ${job.jobId}] ${msg}`);
  };
  ...
  (async () => {
    try {
      ...
      console.error(`[instalacao-encerramento ${job.jobId}] ERRO: ${msg}`);
    } finally {
