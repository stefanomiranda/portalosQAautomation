// core/shared-som/buscarWorklist.js
//
...
//
// NOTA: este arquivo é a versão de Instalação (que funciona), copiado
// para o shared-som/ para ser usado pela Retirada. A Instalação
// continua usando seu próprio steps/buscarWorklist.js (idêntico).
//
// FIX ANTI-TIMEOUT (Retirada): a versão anterior fazia page.goto
// direto para a URL ".local" mesmo quando já estávamos na Worklist.
// Isso causava ERR_CONNECTION_TIMED_OUT porque o DNS .local não
// resolve no PC do usuário (só no servidor). A Instalação tem a
// detecção "isOnWorklist" que pula o goto — esta versão mantém
// a mesma proteção.
//
// FIX ANTI-TIMEOUT 2 (Retirada): adicionado clickWorklistLinkHeader
// como fallback antes do goto (mesmo padrão do worklistHelper).
...
/** Tenta clicar no link "Worklist" do header (mesmo padrão do worklistHelper). */
...
async function buscarWorklist({ page, sa, associatedDocument, onLog = () => {} }) {
  ...
  //    FIX ANTI-TIMEOUT: se não estiver, tenta clicar no link "Worklist"
  //    do header PRIMEIRO (herda sessão + base URL do servidor autenticado);
  //    só usa page.goto como último recurso.
  if (isOnWorklist(page)) {
    log(`já estamos na Worklist (URL: ${page.url()}) — pulando goto`);
  } else {
    log(`não estamos na Worklist (URL=${page.url()}) — tentando clicar no link "Worklist" do header`);
    const clicked = await clickWorklistLinkHeader(page);
    if (clicked) {
      log(`✓ link "Worklist" do header clicado`);
    } else {
      const wlUrl = urlWorklist(page);
      log(`link "Worklist" do header não encontrado — fallback com page.goto: ${wlUrl}`);
  ...
  // 3) Submeter a busca — ENTER primeiro, Refresh como fallback
  ...
  await waitForCondition(
    async () => {
      ...
      const temTarefa = await page.locator('text=/T0(17|37|46|63)/').first().isVisible().catch(() => false);
      return temTarefa;
    },
